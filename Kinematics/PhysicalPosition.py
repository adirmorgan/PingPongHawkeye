...
from utils import *  # assumes toleround, timeit, timing, THREADING_ENABLE, etc.
...
def TOP_3D(
    ...
    frame_index: int,
    full_cfg: dict,
    cam_executor: concurrent.futures.Executor | None = None,
    ...
    # Sequential mode (global toggle)
    if not THREADING_ENABLE:
        res2d = [_call_top2d(cam) for cam in range(n_cameras)]
    else:
        created_cam_executor = False
        if cam_executor is None:
            cam_executor = concurrent.futures.ThreadPoolExecutor(max_workers=min(32, n_cameras))
            created_cam_executor = True

        try:
            res2d = list(cam_executor.map(_call_top2d, range(n_cameras)))
        finally:
            if created_cam_executor:
                cam_executor.shutdown(wait=True)
...
def main():
    with timeit("Setup of main 3D"):
        ...
        # Decide pool sizes (only used when THREADING_ENABLE=True)
        cam_workers = int(phys_cfg.get("top2d_cam_workers", min(32, n_cameras)))
        contour_workers = int(phys_cfg.get("top2d_contour_workers", 16))
        method_workers = int(phys_cfg.get("top2d_method_workers", 3 * max(1, contour_workers)))
        ...
        if THREADING_ENABLE:
            cam_executor = concurrent.futures.ThreadPoolExecutor(max_workers=cam_workers)
